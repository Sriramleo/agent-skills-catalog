...
program.action(async (options) => {
    ...
    if (options.search) {
        ...
        const matches = catalog.skills.filter((s) => s.id.includes(query) ||
    ...
    if (options.list || options.table || options.category || options.tool) {
        ...
        if (options.tool) {
            skills = skills.filter((s) => s.detectedTools.some((t) => t.toLowerCase() === options.tool.toLowerCase()));
        }
    ...
    console.log(pc.green(`✔ Scanned ${pc.bold(initialCatalog.totalSkills)} skills across ${initialCatalog.harnesses.length} harnesses in ${initialCatalog.scanDurationMs}ms.`));
...
program
    ...
    const skill = catalog.skills.find((s) => s.id.toLowerCase() === id.toLowerCase() || s.name.toLowerCase() === id.toLowerCase());
    if (!skill) {
