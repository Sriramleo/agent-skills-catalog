...
`);
    ...
    static renderSummary(catalog) {
        this.renderBanner();
        console.log(`${pc.bold('Total Skills:')} ${pc.green(catalog.totalSkills)}  ${pc.gray('|')}  ${pc.bold('Scan Time:')} ${pc.yellow(`${catalog.scanDurationMs}ms`)}`);
    ...
    static renderTable(skills) {
        console.log(pc.bold(`${'ID'.padEnd(30)} ${'CATEGORY'.padEnd(24)} ${'HARNESS'.padEnd(16)} ${'TOKENS'.padStart(8)}  ${'DESCRIPTION'}`));
