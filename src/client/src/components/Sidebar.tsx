      ...
      {authors && authors.length > 0 && (
        <div className="space-y-1.5 pt-4 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-2 mb-2">
            ...
            <span className="text-[11px] font-mono text-slate-400 dark:text-slate-500">({authors.length})</span>
          </div>

          <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
            {authors.map((a) => {
              const isSelected = selectedAuthor === a.author;
              return (
                <button
                  key={a.author}
                  onClick={() => onSelectAuthor(isSelected ? null : a.author)}
                  className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-xs transition ${
                    isSelected
                      ? 'bg-indigo-50 dark:bg-indigo-600/30 text-indigo-900 dark:text-indigo-200 border border-indigo-300 dark:border-indigo-500/50 font-semibold shadow-sm'
                ...
                >
                  <div className="flex items-center gap-2 truncate pr-2">
                    <User className={`w-3 h-3 shrink-0 ${isSelected ? 'text-indigo-600 dark:text-cyan-400' : 'text-slate-400 dark:text-slate-500'}`} />
                    <span className="truncate">{a.author}</span>
                  </div>
                  <span className={`text-[11px] font-mono shrink-0 ${
                    isSelected
                      ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-500/30 dark:text-indigo-200 px-1.5 py-0.2 rounded'
                      : 'text-slate-500'
                  }`}>
                    {a.count}
                  </span>
