...
export class Categorizer {
  public static categorize(
    ...
    if (explicitCategory) {
      const match = CANONICAL_CATEGORIES.find(
        (c) => c.id === explicitCategory.toLowerCase() || c.name.toLowerCase() === explicitCategory.toLowerCase()
      );
      if (match) {
        return {
          id: match.id,
  ...
  public static extractTags(id: string, description: string, content: string): string[] {
