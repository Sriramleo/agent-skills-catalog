...
export class SkillParser {
  ...
  private static MATT_POCOCK_SKILLS = new Set([
    'ask-matt',
  ...
  private static extractAuthor(
    ...
    if (
      SkillParser.MATT_POCOCK_SKILLS.has(id) ||
  ...
  private static extractFrontmatter(raw: string): { frontmatter: Record<string, any>; content: string } {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        return { frontmatter: parsed as Record<string, any>, content };
      }
    } catch {
  ...
  private static extractWhenToUse(content: string, description: string): string {
  ...
  private static extractHowToUse(content: string): string {
  ...
  private static extractTriggers(content: string, description: string): string[] {
    ...
    for (const match of triggerMatches) {
      if (match[1]) {
        match[1].split(/[,|;]/).forEach(t => {
          ...
          if (clean.length > 2) triggers.add(clean);
        });
      }
    }
    ...
    for (const match of slashMatches) {
      if (match[0] && match[0].length > 2) triggers.add(match[0]);
    }

    return Array.from(triggers).slice(0, 10);
  }

  private static extractPrompts(content: string, id: string, title: string): string[] {
    const prompts = [
  ...
  private static extractWorkflowSnippets(content: string): string[] {
  ...
  private static detectToolsAndMcp(content: string, description: string): string[] {
    const tools = new Set<string>();
    const text = `${description} ${content}`.toLowerCase();

    const KNOWN_TOOLS: { name: string; pattern: RegExp }[] = [
  ...
  private static scanAssets(dirPath: string): SkillAsset[] {
  ...
  private static calculateStats(content: string, assets: SkillAsset[]): SkillStats {
    const lines = content.split('\n');
  ...
  private static getFileMtime(filePath: string): string {
    try {
